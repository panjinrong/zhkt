<template>
	<div class="jspx">
      <P><img :src="img1"><span>{{box_name}}</span><el-button type="primary" @click="box_set">评教设置</el-button></P>
      <div v-if="pd_show">
          <el-table :data="tableData6" border style="width: 100%; margin-top: 20px;color: #555555;font-size: 0.14rem;">
              <el-table-column prop="type" label="评教类型" width="180">
              </el-table-column>
              <el-table-column prop="names" label="评教指标">
      ...
      <div v-else class="jspx_content">
          ...
          <div style="margin-bottom:.2rem;" v-for="(site,index) in sites">
              ...
              <el-collapse-item>
              <template slot="title">
              <p class="jspx_set0_p" @click.top="show_qh(index)"><label>评教类型：</label><span>{{site.name_ktqk}}</span><button @click.stop="deletes(index)">删除类型</button></p>
               </template>
              <el-table 
                      :data="site.tableData"
                      border
